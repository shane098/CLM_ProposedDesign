...
SELECT DISTINCT(a.employee_common_name) As Emp_Name ,  b.loc_id AS loc_id,
		COALESCE(b.HCalls, 0) As Handled_Calls,
		'N/F' As Short_Calls,
		COALESCE(b.TCalls, 0) As Transfered_Calls,
		COALESCE((CAST(b.TCalls AS DECIMAL(38,15)) / b.HCalls) * 100,0) As Transferred_Calls_Percent, 
		COALESCE(b.OCalls, 0) As Outband_Calls ,
		COALESCE((CAST(b.ATalk AS DECIMAL(38,15)) / b.HCalls), 0) As Average_Talk,
		COALESCE((CAST(b.AHold AS DECIMAL(38,15)) / b.HCalls), 0) As Average_Hold,
		COALESCE(b.HoldPercent, 0) As Hold_Percent,
		COALESCE(b.OBPercent, 0) As OB_Percent_of_Talk,
		COALESCE((CAST(b.AAcw AS DECIMAL(38,15)) / b.HCalls), 0) As Average_ACW,
		COALESCE((CAST((b.ATalk + b.AHold + b.AAcw) AS DECIMAL(38,15)) / b.HCalls), 0) As AHT,
		IIF(b.HCalls < 1,0,215400) As Scheduled_Time,
		'N/F' As OT_Time,
		'N/F' As VTO_Time,
		COALESCE(b.STime, 0) As Staffed_Time ,
		COALESCE(b.ATime, 0) As Avail_Time,
		COALESCE(b.IdleTime, 0) As Idle_Time,
		COALESCE(SUM(((CAST((b.ATalk + b.AHold + b.AAcw) AS DECIMAL(38,15)) / b.HCalls) * b.HCalls) / (((b.ATalk + b.AHold + b.AAcw) / b.HCalls) + b.ATime ) )*100, 0) As Occupancy,
		COALESCE((CAST(b.AuxOut AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux_Out_Percent,
		COALESCE(b.AuxOut, 0) As Aux_Out,
		COALESCE((CAST(b.Aux0 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux0_Percent,
		COALESCE(b.Aux0, 0) As Aux0,
		COALESCE((CAST(b.Aux1 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux1_Percent,
		COALESCE(b.Aux1, 0) As Aux1,
		COALESCE((CAST(b.Aux2 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux2_Percent,
		COALESCE(b.Aux2, 0) As Aux2,
		COALESCE((CAST(b.Aux3 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux3_Percent,
		COALESCE(b.Aux3, 0) As Aux3,
		COALESCE((CAST(b.Aux4 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux4_Percent,
		COALESCE(b.Aux4, 0) As Aux4,
		COALESCE((CAST(b.Aux5 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux5_Percent,
		COALESCE(b.Aux5, 0) As Aux5,
		COALESCE((CAST(b.Aux6 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux6_Percent,
		COALESCE(b.Aux6, 0) As Aux6,
		COALESCE((CAST(b.Aux7 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux7_Percent,
		COALESCE(b.Aux7, 0) As Aux7,
		COALESCE((CAST(b.Aux8 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux8_Percent,
		COALESCE(b.Aux8, 0) As Aux8,
		COALESCE((CAST(b.Aux9 AS DECIMAL(38,15)) / b.STime)*100, 0) As Aux9_Percent,
		COALESCE(b.Aux9, 0) As Aux9,
		COALESCE(ROUND((b.TAux / b.STime)*100 , 2), 0) As Total_Aux_Percent,
		COALESCE(b.TAux, 0) As Total_Aux,
		COALESCE((CAST((b.Aux0 + b.Aux8) AS DECIMAL(38,15)) - b.AuxOut), 0) As Net_Aux,
		'N/F' As CCMS_Net_Time_Adjust,
		'N/F' As Absenteeism,
		COALESCE(b.ProdEff, 0) As Product_Efficiency,
		COALESCE((CAST(b.Billable AS DECIMAL(38,15)) / b.STime) * 100, 0) As Bill_to_Pay
		...
		FROM (SELECT a.employee_ident, a.employee_common_name , b.phone_id, a.manager_ident, a.manager_common_name as sup_name,
				...
				WHERE a.program_abbr_name='FRONTIER_AIRLINES_INC_DAVAO_AGENTS' AND 
					  a.status='Active') a
		...
		INNER JOIN (SELECT b.row_date, b.logid, b.loc_id,
					   ...
					   (CAST((SUM(b.i_othertime) + SUM(b.i_acdaux_outtime)) AS DECIMAL(38,15)) / SUM(b.i_acdtime)) * 100 As HoldPercent,
					   CAST((SUM(b.i_auxouttime) - SUM(b.i_acdaux_outtime) + SUM(b.i_acwtime)) AS DECIMAL(38,15)) / SUM(b.i_acdtime)*100 As OBPercent,
					   SUM(b.i_acwtime) + SUM(b.i_acdtime) + SUM(b.holdtime) + SUM(b.i_availtime) As IdleTime,
					   CAST((SUM(b.acdcalls) + SUM(b.i_availtime)) AS DECIMAL(38,15)) / SUM(b.i_stafftime)* 100 As ProdEff,
